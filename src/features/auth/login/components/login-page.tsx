        ...
        <div className="w-full max-w-[420px]">
          ...
          <form className="mt-8 space-y-4">
            <label className="block text-[10px] font-semibold text-foreground">
              Email atau Username
              <input type="text" name="username" autoComplete="username" placeholder="kabitektif@gmail.com" className="mt-2 h-11 w-full rounded-xl border border-border-strong bg-surface px-3 text-xs text-foreground outline-none transition-colors placeholder:text-foreground/40 focus:border-blue" />
            </label>

            <label className="block text-[10px] font-semibold text-foreground">
              <span className="flex items-center justify-between">
                Password
                <Link href="#forgot-password" className="font-normal text-purple transition-opacity hover:opacity-75">Lupa password?</Link>
              </span>
              <span className="relative mt-2 block">
                <input type={showPassword ? "text" : "password"} name="password" autoComplete="current-password" defaultValue="password123" className="h-11 w-full rounded-xl border border-border-strong bg-surface px-3 pr-20 text-xs text-foreground outline-none transition-colors focus:border-blue" />
                <button type="button" onClick={() => setShowPassword((visible) => !visible)} className="absolute inset-y-0 right-3 cursor-pointer text-[10px] font-normal text-purple transition-opacity hover:opacity-75">{showPassword ? "Sembunyikan" : "Lihat"}</button>
              </span>
            </label>

            <button type="submit" className="mt-3 h-11 w-full cursor-pointer rounded-full bg-white text-sm font-semibold text-button-ink transition-colors duration-300 hover:bg-blue">Lanjut Main</button>
          </form>
