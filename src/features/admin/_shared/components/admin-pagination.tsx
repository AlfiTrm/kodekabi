...
export function AdminPagination({ page, totalPages, buildHref, previousLabel = "Prev", nextLabel = "Next" }: AdminPaginationProps) {
  ...
  return (
    <nav className="flex items-center gap-1.5" aria-label="Pagination">
      {page > 1 ? <Link href={buildHref(page - 1)} prefetch={false} scroll={false} className={linkClass}>{previousLabel}</Link> : <span className={disabledClass}>{previousLabel}</span>}
      ...
      {page < totalPages ? <Link href={buildHref(page + 1)} prefetch={false} scroll={false} className={linkClass}>{nextLabel}</Link> : <span className={disabledClass}>{nextLabel}</span>}
    </nav>
  );
}
