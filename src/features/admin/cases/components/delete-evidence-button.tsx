...
import { deleteAdminEvidenceAction } from "../actions/delete-admin-evidence-action";
import type { AdminCase, AdminCaseEvidence } from "../types/admin-case";

const initialState = { error: null };

export function DeleteEvidenceButton({ caseItem, evidence }: { caseItem: AdminCase; evidence: AdminCaseEvidence }) {
  const [open, setOpen] = useState(false);
  const [state, formAction, pending] = useActionState(deleteAdminEvidenceAction, initialState);

  return (
    <>
      <button type="button" onClick={() => setOpen(true)} aria-label={`Hapus ${evidence.label}`} className="grid size-9 cursor-pointer place-items-center rounded-xl border border-border-strong text-foreground/40 transition-colors hover:border-red/50 hover:bg-red/8 hover:text-red">
        <AdminIcon name="delete" className="size-4" />
      </button>

      {open ? (
        <Modal labelledBy="delete-evidence-title" onClose={() => pending ? undefined : setOpen(false)} className="max-w-md">
          <div className="p-6 sm:p-7">
            ...
            <h2 id="delete-evidence-title" className="mt-5 font-display text-2xl font-semibold tracking-[-0.025em]">Hapus evidence?</h2>
            <p className="mt-3 text-sm leading-6 text-foreground/50"><strong className="text-foreground">{evidence.label}</strong> akan dihapus permanen dari case ini.</p>
