...
import { useItemImagePreview } from "../hooks/use-item-image-preview";
...
export function ItemImageUpload({ initialUrl = "", required = false }: ItemImageUploadProps) {
  const { previewUrl, fileName, error, handleImageChange } = useItemImagePreview(initialUrl);
      ...
      <label className="group relative grid min-h-52 cursor-pointer place-items-center overflow-hidden rounded-2xl border border-dashed border-border-strong bg-background transition-colors hover:border-purple/65">
        ...
        {previewUrl ? (
          <>
            ...
            <img src={previewUrl} alt="Pratinjau item" className="absolute inset-0 size-full object-contain p-5" />
