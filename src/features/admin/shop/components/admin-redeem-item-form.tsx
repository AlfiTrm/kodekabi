...
export function AdminRedeemItemForm({ types, item, onCancel }: { types: AdminRedeemType[]; item?: AdminRedeemItem; onCancel?: () => void }) {
  ...
  const { values, setters, valid } = useAdminRedeemItemForm(item, types[0]?.code);
  const typeOptions = types.map((type) => ({ value: type.code, label: type.name }));
      ...
      <div className="grid gap-7 lg:grid-cols-[minmax(0,1.15fr)_minmax(280px,0.85fr)]">
        <div className="space-y-5">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <div><span className="mb-2 block text-xs font-semibold">Tipe</span><AdminFilterSelect name="type" label="Tipe" showLabel={false} value={values.typeCode} onChange={setters.setTypeCode} options={typeOptions.length ? typeOptions : [{ value: "", label: "Tipe belum tersedia" }]} disabled={!typeOptions.length} /></div>
