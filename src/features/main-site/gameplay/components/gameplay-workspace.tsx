...
export function GameplayWorkspace({ sessionId, initialData }: GameplayWorkspaceProps) {
  ...
  function updateAnswer(answer: GameplayAnswer) {
    ...
    saveTimer.current = setTimeout(() => {
      startTransition(async () => {
        const result = await saveGameplayAnswersAction(sessionId, data.session.session_version, Object.values(nextAnswers), crypto.randomUUID());
        if (!result.success) {
...
function InterrogationPanel({ evidence }: { evidence: GameplayEvidence | null }) {
  ...
  const messages = chat?.messages?.length ? chat.messages : [
    { sender: "@korban_vitamin", text: "Awalnya saya kira beneran manjur, tapi setelah seminggu nggak ada efek sama sekali. Malah perut saya jadi mual-mual." },
    { sender: "Kamu", text: "Dari mana kamu tahu soal produk ini?" },
    { sender: "@korban_vitamin", text: "Dari grup WhatsApp keluarga, kak. Ada yang share postingannya @SehatAlami_Id. Katanya banyak yang sudah sembuh." },
    { sender: "Kamu", text: "Ada bukti pembeliannya?" },
    { sender: "@korban_vitamin", text: "Ada kak, saya beli lewat link di bio Instagram mereka. Bayar 350rb lewat transfer. Tidak ada nota resmi, cuma konfirmasi chat." },
  ];
  const quickPrompts = ["Apa efek samping lainnya?", "Sudah lapor ke siapa?", "Ada teman lain yang mengalami?"];

  function sendMessage(message: string) {
    const trimmedMessage = message.trim();
    if (!trimmedMessage) return;
    setUserMessages((current) => [...current, trimmedMessage]);
    setDraft("");
  }
    ...
    <div className="mt-6 flex min-h-0 flex-1 flex-col pb-6">
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
        <div className="flex items-center gap-3 px-1">
          <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-purple text-sm font-bold text-white">K</span>
          <div><strong className="block text-xs">@korban_vitamin</strong><span className="text-[9px] text-foreground/45">Pembeli Vitamin X · <span className="text-green">Bersedia bicara</span></span></div>
        </div>
        {messages.map((message, index) => <div key={`${message.sender}-${index}`} className={`max-w-[90%] rounded-2xl border border-border bg-surface px-3 py-3 text-xs leading-relaxed ${index % 2 ? "ml-auto border-green/60 bg-green/10" : ""}`}>{message.text}</div>)}
        {userMessages.map((message, index) => <div key={`user-${index}`} className="ml-auto max-w-[90%] rounded-2xl border border-green/60 bg-green/10 px-3 py-3 text-xs leading-relaxed">{message}</div>)}
      </div>
      <div className="mt-5 shrink-0">
        <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
          {quickPrompts.map((prompt) => <button key={prompt} type="button" onClick={() => sendMessage(prompt)} className="shrink-0 rounded-full border border-border-strong px-3 py-2 text-[10px] text-foreground/55 transition-colors hover:border-purple hover:text-foreground">{prompt}</button>)}
        </div>
        <p className="mt-2 text-[10px] text-foreground/35">Pilih pertanyaan untuk menggali informasi lebih lanjut</p>
        <form onSubmit={(event) => { event.preventDefault(); sendMessage(draft); }} className="mt-3 flex items-center gap-2 rounded-2xl border border-border bg-surface px-3 py-2 focus-within:border-purple">
          <input value={draft} onChange={(event) => setDraft(event.target.value)} className="min-w-0 flex-1 bg-transparent px-1 py-2 text-xs outline-none placeholder:text-foreground/35" placeholder="Atau tanya sendiri" aria-label="Pertanyaan interogasi" />
          <button type="submit" className="px-1 py-2 text-xs font-semibold text-purple">Kirim</button>
        </form>
      </div>
    </div>
  );
