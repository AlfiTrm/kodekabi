...
export function DetectiveButton({ href, children, sizeClasses, className }: VariantButtonProps) {
  return (
    <Link
      href={href}
      className={`group relative inline-flex items-center justify-center overflow-hidden bg-white font-semibold text-button-ink transition-colors duration-200 group-hover:bg-background group-hover:text-foreground ${sizeClasses.face} ${className}`.trim()}
    >
      <span
        aria-hidden="true"
      ...
      <span
        aria-hidden="true"
        className={`${animationStyles.detectiveLens} pointer-events-none absolute -bottom-6 right-1 z-20 size-12`}
      >
